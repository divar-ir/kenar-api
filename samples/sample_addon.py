from kenar.models.addon import CreatePostAddonRequest, GetUserAddonsRequest, DeleteUserAddonRequest, \
    GetPostAddonsRequest, DeletePostAddonRequest, CreateUserAddonRequest
...
if __name__ == '__main__':
    ...
    title_row = TitleRow(text='این یک نمونه تایتل میباشد', text_color=Color.TEXT_SECONDARY)

    subtitle_row = SubtitleRow(text='این یک سابتایتل میباشد')

    desc_row = DescriptionRow(text='سلام - این یک ویجت تستی میباشد.', has_divider=True, is_primary=True,
                              expandable=False,
                              padded=True, )
    ...
    eval_row = EvaluationRow(
        ...
        left=EvaluationRow.Section(text='سمت چپ', text_color=Color.TEXT_SECONDARY, section_color=Color.SUCCESS_PRIMARY),
        middle=EvaluationRow.Section(text='وسط', text_color=Color.TEXT_SECONDARY, section_color=Color.TEXT_PRIMARY),
        right=EvaluationRow.Section(text='سمت راستی', text_color=Color.TEXT_SECONDARY,
                                    section_color=Color.TEXT_SECONDARY),
    )

    event_row = EventRow(
        title='تایتل', subtitle='سابتایتل',
        has_indicator=False, image_url='', label='لیبل',
        has_divider=True, link='https://www.test.com',
        padded=True, icon=Icon(icon_name=IconName.ADD))

    group_info = GroupInfo(
        has_divider=True,
        items=[GroupInfo.GroupInfoItem(title='تایتل ۱', value='مقدار ۱'),
    ...
    legend_title_row = LegendTitleRow(
        ...
        subtitle='',
        has_divider=True,
        image_url='https://s100.divarcdn.com/static/images/open-platform/CVbl3wzcVeT4jQkZ_jXwCg.jpg',
        tags=[
            LegendTitleRow.Tag(
                text='احراز',
    ...
    selector_row = SelectorRow(
        ...
        has_divider=True,
        has_arrow=True, icon=Icon(icon_name=IconName.INFO),
        link='https://www.test.com',
    )

    wide_button_bar = WideButtonBar(
        button=WideButtonBar.Button(
            title='به سمت سایت شما',
            link='https://www.test.com'),
    )
    ...
    resp = app.addon.create_post_addon(
        ...
        data=CreatePostAddonRequest(
            ...
            widgets=[
                ...
                desc_row, eval_row, event_row, group_info,
                selector_row, wide_button_bar
            ],
        ),
    )
    print(resp)
