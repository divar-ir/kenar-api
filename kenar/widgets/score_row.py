...
class ScoreRow(BaseModel, BaseWidget):
    ...
    def check_score(self) -> Self:
        filled_descriptive_score = self.descriptive_score is not None and len(self.descriptive_score) > 0
        if filled_descriptive_score == bool(self.percentage_score):
            raise ValueError('Exactly one of descriptive_score or percentage_score must be set')
        return self
